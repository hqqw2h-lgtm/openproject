...
function loadMockAccounts() {
  if (!process.env.MOCK_ACCOUNTS_JSON) return defaultAccounts;
  return JSON.parse(process.env.MOCK_ACCOUNTS_JSON);
}
...
const identityRegistry = new JsonFileIdentityRegistry(
  runtimeConfig.identityRegistryPath,
);
const directory = new AccountDirectory(loadMockAccounts(), { identityRegistry });
const states = new ExpiringStateStore();
const secureCookies = new URL(issuer).protocol === 'https:';
...
app.get('/wecom/callback', async (req, res, next) => {
  try {
    ...
    if (!state || typeof req.query.code !== 'string') return res.status(400).send('Invalid or expired WeCom callback');
    const details = await provider.interactionDetails(req, res);
...
function start() {
  app.listen(port, '0.0.0.0', () => {
    console.log(`WeCom Identity Bridge listening on ${port} with issuer ${issuer} (${mode})`);
  });
}
